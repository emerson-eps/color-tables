...
export const ContinuousLegend: React.FC<continuousLegendProps> = ({
  ...
  React.useEffect(() => {
    ...
    async function continuousLegend() {
      ...
      //   const colorScale = isLinear ? scaleLinear().domain([min,max]) : scaleLog().domain([min,max]);
      ...
      //   let xLeg: any = isLinear ? (scaleLinear().domain(reverseRange ? [max, min] : [min, max]).range([10, 158])) : 
      //     scaleLog()
      //     //.domain(reverseRange ? [max, min] : [min, max]).range([10, 158]);
      //   let yLeg: any = isLinear ? (scaleLinear().domain(reverseRange ? [min, max] : [max, min]).range([10, 158])) : 
      //     scaleLog()
      //     //.domain(reverseRange ? [min, max] : [max, min]).range();
      ...
      //     .style("height", 15);
      //const svg = select(divRef.current);
      const colorScale = scaleSymlog().domain([0, 1]).range([0, 150]); // in my case min and max are [0, 1]
      const colorScale2 = scaleLinear().domain([0, 1]).range([0, 150]); // in my case min and max are [0, 1]
      ...
      linearGradient
        ...
        console.log("point------", colorScale(data.offset))
        console.log("data-2", colorScale(data.offset) / 1.5 + "%")
          return colorScale(data.offset) / 1.5 + "%";
        })
        .attr("stop-color", function(data) {
          return data.color;
        });
    ...
    linearGradient2
      ...
      .append("stop")
      .attr("offset", function(data) {
        return colorScale2(data.offset) / 1.5 + "%"; // breakpoint are taken as offset
      })
      .attr("stop-color", function(data) {
        return data.color;
      });
    ...
    // create tick
    svg.append("g").attr("transform", "translate(10,45)").call(axisBottom(colorScale).ticks(4));
    ...
    // create tick
    //svg.append("g").attr("transform", "translate(10,115)").call(axisBottom(colorScale2).ticks(4));
      } catch (error) {
        console.error(error);
      }
    }
