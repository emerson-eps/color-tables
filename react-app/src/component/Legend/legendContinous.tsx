...
import {colorScalesCont,interpolatorContinuous} from "../Utils/d3ColorScale"
...
export const LegendContinous: React.FC<legendProps> = ({
    ...
    React.useEffect(() => {
        ...
        if (useContColorTable == true) {
            colortableLegend(".legend",colorScalesCont)
        } 
        // d3 colors
        else if (useContColorTable == false) {
            colortableLegend1(".legend1",colorScalesCont)
            //d3Legend(".legend1",d3.scaleSequential(legendColor).domain([0,1]));
        }
    }, [useContColorTable]); 

    function colortableLegend(selected_id: string,colorscale: any) {
    ...
    function colortableLegend1(selected_id: string,
        //colorscale: Record<string, (num: number) => string>
        colorscale: any) {
        const itemColor: any = [];
        ...
        var data1 = d3.range(10).map(d=> ({color:itemColor[3](d/10), value:d}))
        // // get the array's min and max value
        var extent: any = d3.extent(data1, d => d.value); 
    ...
    // function d3Legend(selected_id: string, colorscale: d3.ScaleSequential<string, never>) {
    //     const itemColor: ItemColor[] = [];
    //     colorArray.color.forEach((value: [number, number, number, number]) => {
    //         // return the color and offset needed to draw the legend
    //         itemColor.push({
    ...
    return (
        ...
        >
            {useContColorTable ? <div className="legend"></div> : <div className="legend1"></div>}
        </div>
    );
};
